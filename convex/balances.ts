  ...
  switch (effectiveSplitConfig.type) {
    ...
    case 'PERCENTAGES':
      if (expense.splitConfig && expense.splitConfig.type === 'PERCENTAGES') {
        const validSplitUsers = expense.splitConfig.shares.filter((s) =>
          memberIds.some((id) => id === s.userId),
        )
        validSplitUsers.forEach((share) => {
          shares.set(share.userId, expense.amount * share.value)
        })
      } else if (
        group.defaultSplitRatio.type === 'PERCENTAGES' &&
        group.defaultSplitRatio.percentages
      ) {
        const validSplitUsers = group.defaultSplitRatio.percentages.filter(
          (s) => memberIds.some((id) => id === s.userId),
        )
        validSplitUsers.forEach((share) => {
        ...
        console.warn(
          `Percentages not defined correctly for expense ${expense._id}, falling back to EQUAL split.`,
        )
    ...
    case 'FIXED_AMOUNTS':
      if (
        !expense.splitConfig ||
        expense.splitConfig.type !== 'FIXED_AMOUNTS'
      ) {
        console.warn(
          `FIXED_AMOUNTS split called without proper config for expense ${expense._id}, falling back to EQUAL split.`,
        )
...
export const getGroupBalances = query({
  ...
  handler: async (ctx, args): Promise<GroupBalances | null> => {
    ...
    const memberBalancesResult: MemberBalance[] = memberUserDetails.map(
      (member) => ({
        userId: member.userId,
        name: member.name,
