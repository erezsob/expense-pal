...
export function GroupList({ onSelectGroup }: GroupListProps) {
  ...
  if (groups === undefined) {
    return <div className="text-center py-4 text-on-surface-secondary">Loading groups...</div>;
  }

  if (groups.length === 0) {
    return <p className="text-on-surface-secondary text-center py-4">You are not part of any groups yet. Create one above!</p>;
  }
      ...
      {groups.map((group) => (
        <li key={group!._id}
          ...
          <div>
            ...
            <p className="text-sm text-on-surface-secondary">Currency: {group!.currency}</p>
          </div>
          <span className="text-lg text-primary">&#x276F;</span> {/* Right arrow */}
        </li>
      ))}
    </ul>
  );
